...
export function searchBooks(query, limit = 20) {
  const q = norm(query).trim();
  if (!q) return [];
  return sampleBooks
    .filter((b) => {
      const inTitle = norm(b.title).includes(q);
      const inAuthor = norm(b.author).includes(q);
      const inTags = (b.tags || []).some((t) => norm(t).includes(q));
...
export function searchUsers(query, limit = 20) {
  ...
  return usersDirectory
    .filter(
      (u) => norm(u.name).includes(q) || norm(u.username).includes(q)
    )
    .slice(0, limit);
}

export function searchGenres(query, limit = 30) {
  const q = norm(query).trim();
  if (!q) return genreCounts().slice(0, limit);
  // Filter by genre name and return with counts
  return genreCounts()
    .filter((g) => norm(g.name).includes(q))
    .slice(0, limit);
}

// One-call helper for the Search page UI
export function quickSearch(query, limits = { books: 8, users: 6, genres: 10 }) {
  const { books, users, genres } = limits;
  return {
    books: searchBooks(query, books),
    users: searchUsers(query, users),
    genres: searchGenres(query, genres),
  };
}
